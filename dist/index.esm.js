import { WebGLRenderer, Group, PerspectiveCamera, Scene, AmbientLight, SpotLight, SpotLightHelper, AxesHelper, MeshLambertMaterial, Mesh, TextureLoader, BoxGeometry, Vector3, Raycaster, Vector2 } from 'three';
...
function buildCamera(width, height) {
  ...
  var camera = new PerspectiveCamera(50, width / height, 1, 1000);
...
function buildScene() {
  ...
  var scene = new Scene();
  var ambientLight = new AmbientLight(COLOR_AMBIENT_LIGHT, INTENSITY_AMBIENT);
  scene.add(ambientLight);
  var spotLight = new SpotLight(COLOR_SPOTLIGHT, INTENSITY_SPOTLIGHT);
  ...
  if (useHelpers) {
    var spotLightHelper = new SpotLightHelper(spotLight, 0xFF0000);
...
function buildRibbon(textures, _ref) {
  ...
  var objects = textures.map(function (texture, i) {
    ...
    var obj = new Mesh(segmentsGeometries[i], material);
    obj.userData = {
      segment: i
    };
    return obj;
  });
  return (_Group = new Group()).add.apply(_Group, _toConsumableArray(objects));
}
...
function buildRibbonSegmentsGeometry(turnovers, segmentsNumber, radius, segmentHeight, segmentThickness, steps, shiftMultiplier, translateMultiplier) {
  ...
  for (var i = 0; i < segmentsNumber; i++) {
    ...
    var geom = new BoxGeometry(segmentWidth, segmentHeight, segmentThickness, steps, 1, 1);
